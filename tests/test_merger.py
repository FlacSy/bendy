...
# ─── class-level 3-way (field-only classes: DTOs, dataclasses) ──────────────────


class TestClassLevel3Way:
    """A class the user hasn't touched is regenerated wholesale (so new manifest
    fields reach field-only classes with no per-method merge); an edited class
    is preserved; a hand-added class is kept."""

    def test_untouched_dto_is_regenerated(self):
        prev = dedent("""\
        ...
        gen = dedent("""\
            class UserCreate(BaseModel):
                ...
                is_active: bool
        """)
        result = merge(gen, prev, prev_gen=prev)
        assert "is_active" in result  # new field propagated into untouched DTO

    def test_edited_dto_is_preserved(self):
        gen = dedent("""\
            class UserCreate(BaseModel):
                email: str
                password_hash: str
        ...
        user = dedent("""\
            class UserCreate(BaseModel):
                ...
                password: str
        """)
        result = merge(gen, user, prev_gen=gen)
        assert "password:" in result  # user's edit kept
        assert "password_hash" not in result

    def test_hand_added_dto_class_is_kept(self):
        gen = dedent("""\
        ...
        """)
        result = merge(gen, user, prev_gen=gen)
        assert "DeviceCreateResponse" in result and "secret" in result
